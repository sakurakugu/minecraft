...
# -*- coding: utf-8 -*-
"""
1. 这是一个 Python 脚本，用于将 Minecraft 存档文件夹等其他资源文件夹链接到版本文件夹
   以便在不同版本之间共享存档和其他资源。
2. 为什么要编写此脚本
   因为我在使用不同版本的 Minecraft 时，会遇到一些问题，比如：不同版本的 Minecraft 有不同的mod的文件夹，
   比如我1.21.8和1.21.9都是玩原版，但加的辅助mod是冲突的，如果直接开版本隔离的话，
   资源包、光影、存档、备份都是隔离的，不太方便，因此编写了此脚本
3. 已知缺点：
   - 当使用另一台电脑通过网络路径访问使用该脚本更改过的mc后，比如在那台电脑上盘符是Z盘，这台是D盘，则那台
     电脑无法使用这台电脑的资源包、光影、存档、备份等文件夹，因为它们的路径是D盘的，而Z盘没有D盘的文件夹。
     因此，在使用该脚本后，建议不要在另一台电脑上使用这台电脑的资源包、光影、存档、备份等文件夹。
4. 脚本使用方法
   - 确保 Python 已安装在您的计算机上。
   - 通过记事本或VSCode等文本编辑器打开此脚本。
   - 编辑脚本中的 `MC_根目录` 变量，将其设置为您的 Minecraft 根目录（包含 `versions` 文件夹的 `.minecraft` 目录）。
   - 编辑脚本中的 `含mod但也处理的存档目录` 变量，将其设置为您想要处理的存档目录（包含 `mod` 的 `versions` 目录）。
   - 运行脚本。它将创建符号链接，将存档文件夹链接到版本文件夹。
   - 您可以在 `versions` 文件夹中找到链接的存档文件夹。
   
@author Sakurakugu
@date 2025-07-03 05:27:53 (UTC+8) 周四
@change 2025-10-16 22:42:55 (UTC+8) 周六
"""
